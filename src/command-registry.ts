...
export const isGroup = (node: CommandNode): node is GroupCommand => node.kind === 'group';

// ─── Generic arg resolution (used by the REPL parser; yargs parses itself) ──

function coerce(raw: string | boolean, type: PrimType): string | number | boolean {
...
export function resolveLeafArgs(
  ...
  (node.positionals ?? []).forEach((p, i) => {
    ...
    if (raw === undefined) {
      if (p.demandOption) throw new Error(`Missing required argument: <${p.name}>`);
      return;
    }
  ...
  for (const [key, spec] of Object.entries(node.options ?? {})) {
    ...
    if (raw === undefined) {
      if (spec.demandOption) throw new Error(`Missing required option: --${flagName}`);
...
export const COMMANDS: Record<string, CommandNode> = {
  ...
  init: {
    ...
    group: 'App lifecycle',
    positionals: [{ name: 'name', demandOption: true, describe: 'The name of the application to initialize' }],
    options: {
      repo: { alias: 'r', type: 'string', demandOption: true, describe: 'The repository URL of the application' },
      branch: { alias: 'b', type: 'string', default: 'main', describe: 'The branch to use' },
      instances: { alias: 'i', type: 'number', default: 1, describe: 'The number of instances to initialize (default: 1)' },
      port: { alias: 'p', type: 'number', describe: 'The port number to use (default: find an available one)' },
      type: { alias: 't', type: 'string', choices: ['nextjs', 'nestjs', 'dotnet', 'static'], default: 'nextjs', describe: 'The type of application' },
      projectDir: { flag: 'project-dir', alias: 'd', type: 'string', describe: 'Subdirectory within the repo that contains the project (for monorepos)' },
      vcs: { type: 'string', choices: ['git', 'svn', 'local'], default: 'git', describe: 'Version control system to use (use "local" for a local folder path)' },
    },
    lockArg: 'name',
    handler: async ({ name, repo, branch, instances, port, type, projectDir, vcs }) => {
      const appsDir = type === 'nestjs' ? NEST_DIR : type === 'dotnet' ? DOTNET_DIR : type === 'static' ? STATIC_DIR : NEXT_DIR;
      await init({ name, repo, branch, instances, port, type, appsDir, projectDir, vcsType: vcs ?? 'git' });
    },
  },
  ...
  deploy: {
    ...
    group: 'App lifecycle',
    positionals: [{ name: 'name', demandOption: true, describe: 'The name of the application to deploy or update' }],
    options: {
      force: { alias: 'f', type: 'boolean', default: false, describe: 'Force the deployment even if no changes are detected' },
      lint: { alias: 'l', type: 'boolean', default: false, describe: 'Run linting during the deployment process' },
    },
    lockArg: 'name',
  ...
  restart: {
    ...
    group: 'App lifecycle',
    positionals: [{ name: 'name', demandOption: true, describe: 'Application name' }],
    lockArg: 'name',
    handler: async ({ name }) => { await restart({ name }); },
  },
  ...
  stop: {
    ...
    positionals: [{ name: 'name', demandOption: true, describe: 'Application name' }],
    lockArg: 'name',
    handler: async ({ name }) => { await stop({ name }); },
  },
  ...
  rollback: {
    ...
    group: 'App lifecycle',
    positionals: [{ name: 'name', demandOption: true, describe: 'Application name' }],
    options: { to: { type: 'number', describe: 'Build index to roll back to (default: previous)' } },
    lockArg: 'name',
    handler: async ({ name, to }) => { await rollback({ name, to }); },
  },
  ...
  'start-all': {
    ...
    group: 'App lifecycle',
    handler: async () => { await startAllApplications(); },
  },
  ...
  'stop-all': {
    ...
    group: 'App lifecycle',
    handler: async () => { await stopAllApplications(); },
  },
  ...
  info: {
    ...
    group: 'Info & monitoring',
    positionals: [{ name: 'name', demandOption: true, describe: 'Application name' }],
    handler: async ({ name }) => { await info({ name }); },
  },
  ...
  unlock: {
    kind: 'leaf',
    usage: 'unlock <name>',
    describe: 'Forcefully release the lock for an application by killing its process.',
    group: 'Environment',
    positionals: [{ name: 'name', demandOption: true, describe: 'The name of the application' }],
    handler: ({ name }) => { unlock({ name }); },
  },
  ...
  clean: {
    ...
    positionals: [{ name: 'name', demandOption: true, describe: 'The name of the application' }],
    lockArg: 'name',
    handler: async ({ name }) => { await clean({ name }); },
  },

  'clean-all': {
    kind: 'leaf',
    usage: 'clean-all',
    describe: 'Clean all apps: discard uncommitted changes and prune old builds',
    group: 'Environment',
    handler: async () => { await cleanAll(); },
  },
  ...
  storage: {
    ...
    subcommands: {
      new: {
        ...
        group: 'Storage',
        positionals: [
          { name: 'name', demandOption: true, describe: 'The storage directory name (e.g. metalunic-storage)' },
          { name: 'link-name', describe: 'The symlink name created inside each build directory (defaults to name if not provided)' },
        ],
        handler: async ({ name, 'link-name': linkName }) => { await storageNew(name, linkName); },
      },
      attach: {
        kind: 'leaf',
        ...
        group: 'Storage',
        positionals: [
          { name: 'app', demandOption: true, describe: 'The name of the application' },
          { name: 'storage', demandOption: true, describe: 'The name of the storage to attach' },
        ],
        handler: async ({ app, storage }) => { await storageAttach(app, storage); },
      },
      detach: {
        kind: 'leaf',
        ...
        positionals: [
          { name: 'app', demandOption: true, describe: 'The name of the application' },
          { name: 'storage', demandOption: true, describe: 'The name of the storage to detach' },
        ],
        handler: async ({ app, storage }) => { await storageDetach(app, storage); },
      },
      rm: {
        kind: 'leaf',
        usage: 'rm <name>',
        ...
        group: 'Storage',
        positionals: [{ name: 'name', demandOption: true, describe: 'The name of the storage to delete' }],
        handler: async ({ name }) => { await storageRm(name); },
      },
      ls: {
        kind: 'leaf',
        usage: 'ls',
  ...
  domain: {
    ...
    subcommands: {
      add: {
        ...
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name to add' }],
        handler: async ({ name }) => { await domainAdd(name); },
      },
      remove: {
        kind: 'leaf',
        usage: 'remove <name>',
        ...
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name to remove' }],
        options: { force: { type: 'boolean', default: false, describe: 'Cascade delete all routes for this domain' } },
        handler: async ({ name, force }) => { await domainRemove(name, force); },
      },
      list: {
        kind: 'leaf',
        usage: 'list',
        ...
        group: 'Domain',
        handler: async () => { await domainList(); },
      },
      show: {
        kind: 'leaf',
        usage: 'show <name>',
        ...
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name to show' }],
        handler: async ({ name }) => { await domainShow(name); },
      },
      'set-cert': {
        kind: 'leaf',
        usage: 'set-cert <name>',
        ...
        options: {
          cert: { type: 'string', describe: 'Path to certificate file (any extension)' },
          key: { type: 'string', describe: 'Path to private key file (any extension)' },
          pfx: { type: 'string', describe: 'Path to PFX/PKCS#12 bundle (any extension)' },
          password: { type: 'string', describe: 'Password for PFX bundle (use "" for empty)' },
          force: { type: 'boolean', default: false, describe: 'Attach certificate even if it does not cover the domain name' },
        },
        ...
        handler: async ({ name, cert, key, pfx, password, force }) => {
          ...
          if (pfx && password === undefined) throw new Error('--pfx requires --password.');
      ...
      'cert-status': {
        ...
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name' }],
        handler: async ({ name }) => { await domainCertStatus(name); },
      },
      'remove-cert': {
        kind: 'leaf',
      ...
      'reload-certs': {
        ...
        handler: async ({ name }) => { await domainReloadCerts(name); },
      },
      'set-header': {
        kind: 'leaf',
      ...
      'remove-header': {
        ...
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name' }],
        options: { key: { type: 'string', demandOption: true, describe: 'Header name to remove' } },
        handler: async ({ name, key }) => { await domainRemoveHeader(name, key); },
      },
      compile: {
        kind: 'leaf',
        usage: 'compile <name>',
        ...
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name' }],
        handler: async ({ name }) => { await domainCompile(name); },
      },
      'show-config': {
        kind: 'leaf',
        usage: 'show-config <name>',
        describe: 'Preview the Nginx config for a domain without writing to disk',
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name' }],
        handler: async ({ name }) => { await domainShowConfig(name); },
      },
      push: {
        kind: 'leaf',
        usage: 'push <name>',
        describe: 'Push compiled domain config to Nginx (local or remote). Env vars: NGINX_REMOTE_HOST, NGINX_REMOTE_KEY, PUSH_CERT_DIR',
        group: 'Domain',
        positionals: [{ name: 'name', demandOption: true, describe: 'The domain name' }],
        handler: async ({ name }) => { await domainPush(name); },
      },
    },
  },
  ...
  route: {
    ...
    subcommands: {
      add: {
        ...
        group: 'Route',
        positionals: [
          { name: 'appName', demandOption: true, describe: 'The name of the application' },
          { name: 'domainName', demandOption: true, describe: 'The domain name' },
        ],
        options: {
          location: { alias: 'l', type: 'string', default: '', describe: 'Route path without leading slash (e.g. "api", "admin/dashboard"). Root = omit or leave empty.' },
          force: { alias: 'f', type: 'boolean', default: false, describe: 'Allow routing the app even if it is already routed elsewhere' },
        },
        handler: async ({ appName, domainName, location, force }) => { await routeAdd(appName, domainName, location, force); },
      },
      remove: {
        kind: 'leaf',
        ...
        options: { location: { alias: 'l', type: 'string', default: '', describe: 'Route path without leading slash (e.g. "api"). Root = omit or leave empty.' } },
        handler: async ({ domainName, location }) => { await routeRemove(domainName, location); },
      },
      list: {
        kind: 'leaf',
        ...
        group: 'Route',
        positionals: [{ name: 'domainName', demandOption: true, describe: 'The domain name' }],
        handler: async ({ domainName }) => { await routeList(domainName); },
      },
      'set-header': {
        kind: 'leaf',
      ...
      'remove-header': {
        ...
        options: {
          location: { alias: 'l', type: 'string', default: '', describe: 'Route path without leading slash (e.g. "api"). Root = omit or leave empty.' },
          key: { type: 'string', demandOption: true, describe: 'Header name to remove' },
        },
        handler: async ({ domainName, location, key }) => { await routeRemoveHeader(domainName, location, key); },
      },
    },
  },
  ...
  'change-repo': {
    ...
    lockArg: 'name',
    handler: async ({ name, repo }) => { await changeRepo({ name, newRepo: repo }); },
  },
