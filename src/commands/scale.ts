...
export const scale = async (params: ScaleParams) => {
  ...
  if (updates.instances !== undefined) Logger.info(`  Instances: ${app.config.instances} → ${Logger.highlight(updatedConfig.instances.toString())}`);
  if (updates.maxMemory !== undefined) Logger.info(`  Max Memory: ${app.config.maxMemory} → ${Logger.highlight(updatedConfig.maxMemory)}`);
  if (updates.autorestart !== undefined) Logger.info(`  Auto Restart: ${app.config.autorestart ?? 'default'} → ${Logger.highlight(String(updatedConfig.autorestart))}`);
  if (updates.maxRestarts !== undefined) Logger.info(`  Max Restarts: ${app.config.maxRestarts ?? 'default'} → ${Logger.highlight(updatedConfig.maxRestarts?.toString() ?? 'default')}`);
  if (updates.minUptime !== undefined) Logger.info(`  Min Uptime: ${app.config.minUptime ?? 'default'} → ${Logger.highlight(updatedConfig.minUptime ?? 'default')}`);
  if (updates.restartDelay !== undefined) Logger.info(`  Restart Delay: ${app.config.restartDelay ?? 'default'} → ${Logger.highlight(updatedConfig.restartDelay?.toString() ?? 'default')}ms`);
  if (updates.killTimeout !== undefined) Logger.info(`  Kill Timeout: ${app.config.killTimeout ?? 'default'} → ${Logger.highlight(updatedConfig.killTimeout?.toString() ?? 'default')}ms`);
  if (updates.nodeArgs !== undefined) Logger.info(`  Node Args: ${app.config.nodeArgs ?? 'none'} → ${Logger.highlight(updatedConfig.nodeArgs ?? 'none')}`);
