...
export function createMetricsRunner({ name }: { name: string }): StreamingRunner {
  return {
    start: (emit) => {
      const stopFns: Array<() => void> = [];
      let cleanedUp = false;
      ...
      const init = async () => {
        try {
          ...
          for (const d of unpushedDomains) {
            emit(`\x1b[90m[${new Date().toLocaleTimeString()}]\x1b[0m \x1b[33m⚠\x1b[0m Domain "${d}" has never been pushed to Nginx — skipping (run: dm domain push ${d})`);
          }
          ...
          for (const { logPath, label } of targets) {
            if (isRemote) {
              const stop = await tailRemoteForRunner(logPath, multiLabel ? label : undefined, emit);
              stopFns.push(stop);
            } else {
              const stop = tailLocalForRunner(logPath, multiLabel ? label : undefined, emit);
              stopFns.push(stop);
            }
          }
        } catch (err: any) {
          emit(`\x1b[31m✖\x1b[0m ${err?.message ?? String(err)}`, 'stderr');
          cleanup();
        }
      };
...
function tailLocalForRunner(logPath: string, label: string | undefined, emit: (line: string) => void): () => void {
  const child = spawn('tail', ['-f', '-n', '50', logPath], {
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  ...
  child.stdout.on('data', (chunk: Buffer) => {
    ...
    for (const line of lines) {
      emit(formatLogLine(line, label));
    }
  });

  child.stderr.on('data', (chunk: Buffer) => {
    emit(`\x1b[90m[${new Date().toLocaleTimeString()}]\x1b[0m \x1b[33m⚠\x1b[0m tail: ${chunk.toString().trim()}`);
  });

  child.on('error', (err) => {
    emit(`\x1b[90m[${new Date().toLocaleTimeString()}]\x1b[0m \x1b[31m✖\x1b[0m tail error: ${err.message}`);
  });

  return () => child.kill();
}

async function tailRemoteForRunner(
  logPath: string,
  label: string | undefined,
  ...
  const stop = conn.execStream(
    cmd,
    (line) => emit(formatLogLine(line, label)),
    (err) => emit(`\x1b[31m✖\x1b[0m ${err.message}`)
  );

  return stop;
}

function formatLogLine(line: string, label?: string): string {
  const trimmed = line.trim();
  if (!trimmed) return '';
  ...
  if (entry) {
    ...
    return `${ts} ${prefix}${statusStr} ${entry.method} ${entry.uri}${rt} - ${entry.remoteAddr}`;
  }
...
/**
 * Launches the metrics streaming view.
 * Works in both REPL (navigation push) and CLI (direct bootstrapApp) contexts.
 */
export async function launchMetrics(name: string): Promise<void> {
