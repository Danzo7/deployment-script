...
const npmInstall = spawnSync('npm', ['install'], { cwd: dir, stdio: 'inherit', shell: true });
...
const nextBuild = spawnSync('npx', ['next', 'build'], { cwd: dir, stdio: 'inherit', shell: true });
...
const executePM2Commands = (appName:string,dir:string) => {
  // Delete PM2 process
  const deleteProcess = spawnSync('npx', ['pm2','delete', appName],{ cwd: dir, stdio: 'inherit',shell: true });
  ...
  // Start PM2 process
  const startProcess = spawnSync('npx', ["pm2",'start'],{ cwd: dir, stdio: 'inherit',shell: true });
...
const cleanupOldReleases = (releaseDir:string, releasesDir:string) => {
  try {
      ...
      const oldReleases = allReleases.filter(release => path.join(releasesDir,release)!==releaseDir);
...
const deploy = async (appName:string,appDir: string, repo: string, envDir?: string) => {
  try {
  ...
  if(!isFirstDeploy&&await simpleGit(currentDir).checkIsRepo() && await simpleGit(releaseDir).checkIsRepo()) {
