...
const nextjsHandler: AppTypeHandler = {
  typeKey: 'nextjs',
  ...
  createBuildDir({ appDir, projectDir, storages }: CreateBuildDirInput): string {
    ...
    if (fs.existsSync(publicFolder)) fsExtra.copySync(publicFolder, path.join(buildDir, 'public'));
    if (fs.existsSync(contentFolder)) {
      Logger.info('Content folder found and being preserved...');
      fsExtra.copySync(contentFolder, path.join(buildDir, 'content'));
    }
    if (!fs.existsSync(nextFolder)) throw new Error('Next.js build not found.');

    fs.mkdirSync(buildDir, { recursive: true });

    const nodeModulesSrc = path.join(sourceRoot, 'node_modules');
    if (!fs.existsSync(nodeModulesSrc)) throw new Error('Node modules not found.');
    ...
    if (fs.existsSync(envLocalSrc)) fs.copyFileSync(envLocalSrc, path.join(buildDir, '.env.local'));
    ...
    for (const ext of nextConfigExts) {
      ...
      if (fs.existsSync(src)) fs.copyFileSync(src, path.join(buildDir, 'next.config' + ext));
    }
  ...
  async detect(dir: string): Promise<number> {
    for (const ext of ['.js', '.mjs', '.ts', '.cjs']) {
      if (fs.existsSync(path.join(dir, `next.config${ext}`))) return 95;
    }
