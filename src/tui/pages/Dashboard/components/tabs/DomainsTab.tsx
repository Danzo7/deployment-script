...
import { DETAIL_W } from '../../../../utils/constants.js';
import { fmtDate, truncate } from '../../../../utils/formatters.js';
...
export function DomainsTab({
  detail,
  maxVisible,
}: DomainsTabProps): React.ReactElement {
  if (detail === null) {
    return (
      <Box width={DETAIL_W}>
      ...
      {domains.map((domain, idx) => (
        ...
        >
          ...
          {domain.cert.issuer && (
            <Box marginLeft={2}>
              <Text dimColor>
                issuer: {truncate(domain.cert.issuer, DETAIL_W - 12)}
              </Text>
            </Box>
          )}
          <Box marginLeft={2}>
