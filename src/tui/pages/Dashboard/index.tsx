/**
 * Dashboard index.tsx — layout shell + input handler.
 * All data concerns live in launch.tsx.
...
} from '../../components/dialogs/CommandPalette.js';
import { ConfirmDialog } from '../../components/dialogs/ConfirmDialog.js';
import { CrashLoopToast } from '../../components/notifications/CrashLoopToast.js';
...
import type { DetailTab, ActionMode, DashboardAction } from './types.js';

// Re-export types for external use
export type { DetailTab, ActionMode, DashboardAction };
    ...
    <Box flexDirection="row" justifyContent="space-between" width={DETAIL_W}>
      <Box flexDirection="row" gap={1}>
        <Text bold color="white">
          {truncate(summary.app.name, Math.floor(DETAIL_W * 0.5))}
        </Text>
...
export function Dashboard(props: DashboardProps): React.ReactElement {
  ...
  const filteredSummaries = filterQuery.trim()
    ? summaries.filter((s) =>
        s.app.name.toLowerCase().includes(filterQuery.toLowerCase())
      )
    : summaries;
  ...
  useInput((input, key) => {
      ...
      if (input === 'y' || key.return) {
        if (selectedSummary) {
          if (actionMode === 'confirm-restart')
            ...
            const activeIdx =
              selectedSummary.app.builds?.findIndex(
    ...
    if (input === '/') {
      ...
      if (tab === 'deploys')
        setDeployCursor((d) =>
          Math.min((selectedSummary?.app.builds?.length ?? 1) - 1, d + 1)
        );
      ...
      setTabScrollOffset((o) =>
        Math.max(0, o - Math.max(1, Math.floor(DETAIL_H / 2)))
      );
    } else if (input === 'r') {
  ...
  const { pm2Reachable, dbReachable, sshReachable, sshHost, totalMemBytes } =
    props.globalState;
  const cpuHistory =
    cpuHistories.current.get(selectedSummary?.app.name ?? '') ?? [];
  const memHistory =
    memHistories.current.get(selectedSummary?.app.name ?? '') ?? [];
    ...
    <Box flexDirection="column" width={TERM_W}>
      ...
      {toastAppName && (
        <CrashLoopToast
          ...
          restartCount={
            summaries.find((s) => s.app.name === toastAppName)?.restartDelta ??
            0
          }
          tickCount={toastTick}
          onDismiss={() => {
