...
import type { AppSummary, AppDetail, VcsDriftInfo } from '../../../utils/dashboard-data.js';
import type { DomainInfo } from '../../../utils/dashboard-data.js';
import { DETAIL_W, fmtMem, fmtUptime, fmtDate, truncate, pad } from '../shared.js';
...
export function OverviewTab({ summary, detail }: OverviewTabProps): React.ReactElement {
  ...
  for (const d of domains) {
    ...
    for (const r of d.routes) {
      ...
      if (d.cert.mode !== 'none') {
        ...
        else if (d.cert.expiringSoon) sslLabel = `SSL expires ${d.cert.daysRemaining}d`;
        else if (d.cert.daysRemaining !== undefined) sslLabel = `SSL valid · ${d.cert.daysRemaining}d`;
        else sslLabel = d.cert.mode;
      }
      allRoutes.push({ url: `${protocol}://${d.name}${pathPart}`, sslLabel, certValid: !d.cert.isExpired && d.cert.mode !== 'none' });
    }
  }

  const KVItem = ({ label, value, valueColor }: { label: string; value: string; valueColor?: string }) => (
    <Box flexDirection="row" gap={1}>
      <Text dimColor>{pad(label + ':', 12)}</Text>
      {valueColor ? <Text color={valueColor}>{truncate(value, DETAIL_W / 2 - 14)}</Text> : <Text>{truncate(value, DETAIL_W / 2 - 14)}</Text>}
    </Box>
  );
    ...
    <Box flexDirection="column" width={DETAIL_W} gap={1}>
      <Box flexDirection="row" gap={2}>
        <Box flexDirection="column" width={half}>
          ...
          <KVItem label="Uptime"    value={pm2?.status === 'online' ? fmtUptime(pm2.uptimeMs) : '—'} />
          <KVItem label="Exec Mode" value={pm2?.execMode ?? '—'} />
          <KVItem label="PID"       value={pm2?.pid != null ? String(pm2.pid) : '—'} />
          {portReachable !== undefined && (
            <KVItem label="Port check" value={portReachable ? 'reachable' : 'unreachable'} valueColor={portReachable ? 'green' : 'red'} />
          )}
        </Box>
        <Box flexDirection="column" width={half}>
          ...
          <KVItem label="Active Build" value={app.activeBuild ? (app.activeBuild.replace(/[/\\]+$/, '').split(/[/\\]/).pop() ?? '—') + (app.builds?.length ? ` (${app.builds.length})` : '') : '—'} />
      ...
      <Box flexDirection="row" gap={2}>
        ...
        {firstCert ? (
          <Text color={certBadgeColor(firstCert.daysRemaining)}>
            {firstCert.daysRemaining !== undefined ? `[cert ${firstCert.daysRemaining}d]` : '[cert —]'}
          </Text>
      ...
      {detail === null && (
        <Box><Text dimColor>loading detail…</Text></Box>
      )}

      <Box flexDirection="column">
        <Text dimColor>Last commit</Text>
        <Box marginLeft={2}><Text dimColor>{truncate(commit?.message ?? '—', DETAIL_W - 4)}</Text></Box>
        <Box flexDirection="row" gap={2} marginLeft={2}>
          <Text>{commit?.author ?? '—'}</Text>
          <Text dimColor>{commit?.date ? fmtDate(new Date(commit.date)) : '—'}</Text>
        </Box>
      </Box>
      ...
      <Box flexDirection="column">
        ...
        {allRoutes.length === 0 ? (
          <Box marginLeft={2}><Text dimColor>{detail === null ? 'loading…' : 'No routes configured'}</Text></Box>
        ) : allRoutes.map((r, i) => (
          <Box key={i} flexDirection="row" gap={1} marginLeft={2}>
            <Text color="magenta">{truncate(r.url, DETAIL_W - 20)}</Text>
            <Text dimColor color={r.certValid ? 'green' : r.sslLabel === 'no SSL' ? undefined : 'yellow'}>{r.sslLabel}</Text>
          </Box>
        ))}
      </Box>
    </Box>
