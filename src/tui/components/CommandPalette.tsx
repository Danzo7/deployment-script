...
export function parsePaletteInput(input: string, appNames: string[]): DashboardAction | null {
  ...
  if (!appName || !(PALETTE_VERBS as readonly string[]).includes(verb)) return null;
...
interface CommandPaletteProps { input: string; appNames: string[]; }

export function CommandPalette({ input, appNames }: CommandPaletteProps): React.ReactElement {
  const paletteWidth = Math.min(60, TERM_W - 4);
  const suggestions = getPaletteSuggestions(input, appNames);

  return (
    <Box flexDirection="column" alignSelf="center" width={paletteWidth} borderStyle="round" borderColor="yellow">
      ...
      {suggestions.map((s, i) => (
        <Box key={s} flexDirection="row">
          {i === 0
            ? <><Text bold color="yellow">▶ </Text><Text bold color="white">{s}</Text></>
            : <><Text dimColor>{'  '}</Text><Text dimColor>{s}</Text></>}
        </Box>
      ))}
