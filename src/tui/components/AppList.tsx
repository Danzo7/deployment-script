  ...
  return (
    <Box flexDirection="column" width={LIST_W}>
      <Box flexDirection="row" width={LIST_W}>
        {selected && <Text bold color="yellow">▌▌</Text>}
      ...
      <Box flexDirection="row" width={LIST_W} marginLeft={selected ? 3 : 1}>
        {restartDelta >= 3
          ...
          : <Text dimColor>{pm2 ? `${pm2.cpu.toFixed(1)}%  ${fmtMem(pm2.memBytes)}` : '—'}</Text>}
      </Box>
    </Box>
  );
}
...
export function AppList({ summaries, cursor }: AppListProps): React.ReactElement {
  ...
  return (
    <Box flexDirection="column" width={LIST_W}>
      {visible.map((s, i) => (
        <AppRow key={s.app.name} data={s} selected={viewOffset + i === cursor} />
      ))}
