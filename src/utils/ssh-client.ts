...
function assertSshAvailable(): void {
  ...
  if (result.error) {
    ...
    if (process.platform === 'win32') {
      console.log(chalk.gray('  Settings -> Apps -> Optional Features -> OpenSSH Client'));
      console.log(chalk.gray('  Or run in PowerShell (as Admin):'));
      console.log(chalk.yellow('    Add-WindowsCapability -Online -Name OpenSSH.Client~~~~0.0.1.0'));
    } else {
      console.log(chalk.gray('  Install via your package manager, e.g.:'));
      console.log(chalk.yellow('    apt install openssh-client   # Debian/Ubuntu'));
...
export async function connectRemote(host: string, port?: number, identity?: string): Promise<void> {
  ...
  if (exitCode === 255) {
    ...
    console.log(chalk.gray(`  - Server is not reachable at ${cleanHost}:${targetPort}`));
    console.log(chalk.gray('  - Your public key is not authorized on the server'));
    console.log('');
