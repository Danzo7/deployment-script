...
async function promptForType(prompt: string, options: string[]): Promise<string> {
 ...
 */
export async function detectProjectType(opts: DetectAppTypeOptions): Promise<ProjectType> {
  ...
  try {
    ...
    if (!process.stdin.isTTY) throw new Error(`${msg} Pass --type explicitly.`);
    Logger.warn(msg);
    return (await promptForType('Please choose the app type:', getRegisteredTypes())) as ProjectType;
  } finally {
