...
export function extractPfx(opts: {
  ...
  try {
    // Extract certificate chain (no private keys, client certs only)
    execSync(
      `openssl pkcs12 -in "${pfxPath}" -nokeys -clcerts -out "${tmpCertPath}" -passin "file:${passTmpFile}" -legacy`,
      { stdio: 'pipe' }
    );

    // Extract private key (no certs, no encryption on output key)
    execSync(
      `openssl pkcs12 -in "${pfxPath}" -nocerts -nodes -out "${tmpKeyPath}" -passin "file:${passTmpFile}" -legacy`,
      { stdio: 'pipe' }
    );
  } catch (err: any) {
    throw new Error(
