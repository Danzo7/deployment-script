...
import { COMMANDS, isGroup, ensureAppDirectories, } from './command-registry.js';
import { createRequire } from 'module';
import { fileURLToPath } from 'url';
import { dirname, resolve } from 'path';
const _require = createRequire(import.meta.url);
const _pkg = _require(resolve(dirname(fileURLToPath(import.meta.url)), '../package.json'));
...
function registerNode(y, key, node) {
    ...
    return y.command(node.usage, node.describe, (yb) => buildLeafYargs(yb, node), wrapHandler(node));
}
