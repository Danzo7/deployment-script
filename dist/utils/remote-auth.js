...
import { REMOTE_DIR, REMOTE_AUTHORIZED_KEYS_PATH, REMOTE_LOGIN_ATTEMPTS_PATH, REMOTE_AUDIT_LOG_PATH, } from '../constants.js';
// ── Directory bootstrap ──────────────────────────────────────────────────────
...
function parseAuthorizedKeysFile() {
    if (!fs.existsSync(REMOTE_AUTHORIZED_KEYS_PATH))
        return [];
    const lines = fs
 ...
 */
export function addAuthorizedKey(keyOrPath, username) {
    ensureRemoteDir();
    ...
    if (!ALLOWED_KEY_TYPES.has(keyType)) {
        throw new Error(`Key type "${keyType}" is not allowed. Use one of: ed25519, sk-ssh-ed25519 (FIDO2), ecdsa-sha2-nistp256/384/521, sk-ecdsa-sha2-nistp256 (FIDO2), or RSA ≥ 4096 bits`);
    }
    ...
    if (keyType === 'ssh-rsa') {
        ...
        if (bitLength < RSA_MIN_BITS) {
            throw new Error(`RSA key is ${bitLength} bits. Minimum accepted size is ${RSA_MIN_BITS} bits. Use an ed25519 key for best security.`);
        }
    }
    const fp = computeFingerprint(parsed.getPublicSSH());
...
export function removeAuthorizedKey(fingerprint) {
    const existing = parseAuthorizedKeysFile();
...
export function removeAuthorizedKeyByUsername(username) {
    const existing = parseAuthorizedKeysFile();
...
export function recordFailedAttempt(ip) {
    ...
    const backoffMs = Math.min(Math.pow(2, rec.fails) * 1000, 600000);
