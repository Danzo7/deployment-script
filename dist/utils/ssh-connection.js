...
export class SshConnection {
     ...
     */
    async run(command, stdin, timeoutMs = 30000, signal) {
        if (!this.connected) {
        ...
        try {
            ...
            try {
                if (this.effectiveSudoPassword) {
                    return await this.exec(`sudo -S -p '' ${command}`, this.effectiveSudoPassword + '\n');
                }
        ...
        this.client.exec(fullCommand, (err, stream) => {
            ...
            stream.on('close', () => {
                ...
                if (buf.length > 0)
                    onLine(buf);
                if (!stopped)
                    onError(new Error('Remote stream closed unexpectedly'));
            });
        });
        return () => {
            stopped = true;
        ...
        return new Promise((resolve, reject) => {
            ...
            this.client.sftp((err, sftp) => {
                ...
                sftp.fastPut(localPath, remotePath, (putErr) => {
                    ...
                    clearTimeout(timer);
                    if (putErr) {
                        reject(new Error(`Failed to transfer ${localPath} to ${this.creds.remoteHost}: ${putErr.message}`));
                    }
                    else {
                        resolve();
                    }
