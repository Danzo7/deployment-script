...
export const isDirectoryEmpty = (dir) => fs.existsSync(dir) &&
    fs.statSync(dir).isDirectory() &&
    fs.readdirSync(dir).length === 0;
export const ensureDirectories = (appDir) => {
 ...
 */
export const hasPackageJson = (dir) => fs.existsSync(path.join(dir, 'package.json'));
/**
 ...
 */
export const applyStorageSymlinks = (buildDir, storages = []) => {
        ...
        if (stat !== null) {
            if (stat.isSymbolicLink()) {
                ...
                if (existingTarget === targetPath) {
                    ...
                    Logger.error(`applyStorageSymlinks: stale symlink at "${linkPath}" points to "${existingTarget}" instead of "${targetPath}". Skipping.`);
                    continue;
                }
            }
            else {
                Logger.warn(`applyStorageSymlinks: a real file or directory already exists at "${linkPath}" (storage: "${storage.name}"). Skipping.`);
                continue;
            }
        }
