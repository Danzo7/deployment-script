...
/**
 * Clones the repo into a temporary staging directory, runs detection against
 * the appropriate subdirectory, then cleans up.
 * Returns the detected (or user-chosen) ProjectType.
 */
export async function detectProjectType(opts) {
    ...
    try {
        ...
        if (projectDir) {
            ...
            if (fs.existsSync(subDir)) {
                ...
                Logger.warn(`--project-dir "${projectDir}" does not exist in the cloned repo. Falling back to repo root for detection.`);
            }
        }
        ...
        if (result.ambiguous && result.candidates) {
            ...
            const msg = `App type is ambiguous (${top.projectType}: ${top.score}, ${second.projectType}: ${second.score}).`;
            if (!process.stdin.isTTY)
                throw new Error(`${msg} Pass --type explicitly.`);
            Logger.warn(msg);
            return (await promptForType('Please choose the app type:', result.candidates.map((c) => c.projectType)));
        }
