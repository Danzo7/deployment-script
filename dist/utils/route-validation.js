 ...
 */
export function normalizeDomainName(s) {
    let result = s;
 ...
 */
export function validateHostname(s) {
    if (!s || s.length === 0)
        return false;
 ...
 */
export function assertPathUnique(routes, domainId, normalizedPath, domainName) {
 ...
 */
export function assertAppUniqueOnDomain(routes, domainId, app, domainName) {
 ...
 */
export function assertAppNotRoutedElsewhere(routes, app, domains) {
    ...
    if (existing) {
        ...
        throw new Error(`App "${app.name}" is already routed at ${domainName}${displayPath}. Use --force to add another route.`);
    }
}
