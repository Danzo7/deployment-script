...
import { constructSitesAvailablePath, constructSitesEnabledPath, buildRollbackTargets, shellQuote, } from './domain-push-utils.js';
import { validateSafeDomainName, validateCertPath, generateSecureTempFilename, } from './security-validation.js';
...
export class RemotePusher extends NginxPusher {
    ...
    async ensureLogFormatSnippet() {
        ...
        try {
            ...
            await this.ssh.execWithSudo(`sh -c ${shellQuote(`mkdir -p ${path.dirname(DM_LOG_FORMAT_SNIPPET_PATH)} && mv ${shellQuote(tempFile)} ${shellQuote(DM_LOG_FORMAT_SNIPPET_PATH)}`)}`);
        }
        catch (err) {
    ...
    async rollback(snapshot) {
        ...
        throw new Error(`CRITICAL: Rollback failed and Nginx state may be inconsistent. Manual intervention required.\n${parts.join('\n')}`);
    }
    /**
