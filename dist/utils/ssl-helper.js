 ...
 */
export function checkOpenssl() {
    try {
        execSync('openssl version', { stdio: 'pipe' });
    }
    catch {
        throw new Error('openssl is not installed or not on your PATH. Install it and re-run.');
    }
}
/**
...
export function extractPfx(opts) {
    ...
    try {
        ...
        const runExtract = (providerFlags) => {
            execSync(`${baseCmd} -nokeys -clcerts -out "${tmpCertPath}"${providerFlags}`, { stdio: 'pipe' });
            execSync(`${baseCmd} -nocerts -nodes -out "${tmpKeyPath}"${providerFlags}`, { stdio: 'pipe' });
        };
        try {
            ...
            if (stderr.includes('Mac verify error') ||
        ...
        if (err.message === 'Invalid PFX password')
            throw err;
        throw new Error(`PFX extraction failed: ${err.stderr?.toString() ?? err.message}`);
    }
    finally {
 ...
 */
export function parseCertMetadata(certPem) {
    const cert = new X509Certificate(certPem);
    const expiresAt = toISO(cert.validTo);
    const issuedTo = cert.subject
        .split('\n')
 ...
 */
export function validateCert(certPem) {
    let cert;
    try {
 ...
 */
export function validateKey(keyPem) {
    try {
 ...
 */
export function validateKeyMatchesCert(certPem, keyPem) {
    const cert = new X509Certificate(certPem);
 ...
 */
export function certCoversHostname(certPem, hostname) {
    const cert = new X509Certificate(certPem);
    ...
    if (sans.length > 0) {
        for (const san of sans) {
            ...
            if (san.startsWith('*.')) {
                ...
                if (hostLabels.length === parentLabels.length + 1 &&
 ...
 */
export function getCertStorePath(domainName) {
 ...
 */
export function deleteCertFiles(domainName) {
    const certStoreDir = getCertStorePath(domainName);
 ...
 */
export function validateCertAndKey(certPem, keyPem) {
 ...
 */
export function loadCertFromStore(domainName) {
    const certStoreDir = getCertStorePath(domainName);
