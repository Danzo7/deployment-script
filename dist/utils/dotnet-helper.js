 ...
 */
export const checkDotnetInstalled = () => {
    try {
        execSync('dotnet --version', { stdio: 'pipe', encoding: 'utf8' });
        return true;
    }
    catch {
        Logger.warn('.NET SDK is not installed or not on your PATH.\n' +
 ...
 */
export const checkDotnetSdk = async (relDir) => {
    ...
    try {
        ...
        throw new Error("dotnet SDK not found. Install the .NET SDK from https://dotnet.microsoft.com/download and ensure 'dotnet' is on your PATH.");
    }
    // 2. Find .csproj
    const csprojPath = findCsprojFile(relDir);
    if (!csprojPath) {
        throw new Error('No .csproj file found in the repository. Make sure your .NET project has a .csproj file at the repository root.');
    }
    ...
    if (requiredMajor !== null && installedMajor < requiredMajor) {
        throw new Error(`dotnet SDK version mismatch: project requires .NET ${requiredMajor} but found ${installedMajor}. Upgrade the SDK from https://dotnet.microsoft.com/download.`);
    }
};
/**
 ...
 */
export const checkAppSettings = async (relDir, envDir) => {
 ...
 */
export const ensureAssemblyName = async (relDir, appName) => {
    // 1. Find .csproj
    const csprojPath = findCsprojFile(relDir);
    if (!csprojPath) {
        throw new Error('No .csproj file found in the repository. Make sure your .NET project has a .csproj file at the repository root.');
    ...
    if (assemblyNameMatch) {
        ...
        if (projectFile !== appName) {
            ...
            if (propertyGroupMatch) {
                ...
                throw new Error('Could not find <PropertyGroup> in .csproj file. Please add <AssemblyName> manually.');
            }
        }
    }
};
 ...
 */
export const prepareDotnet = async (dir, opts) => {
