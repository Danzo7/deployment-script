 ...
 */
export function compileDmLogFormatSnippet() {
    return (`log_format ${DM_LOG_FORMAT_NAME} '{"ts":"$time_iso8601","method":"$request_method",` +
...
function buildLocationBlocks(domain, routes, hasSsl) {
    ...
    return sorted
        ...
        const safeRoute = route.path.replace(/[^a-z0-9]/gi, '_').replace(/^_+|_+$/g, '') ||
            'root';
        ...
        const lines = [
            ...
            ...getHandler(route.app.projectType).getNginxLocationDirectives().map((d) => `${INDENT}${INDENT}${d};`),
        ];
 ...
 */
export function compileDomainConfig(domain, routes, allDomains) {
 ...
 */
export async function resolveNginxConfig(name) {
    ...
    if (isApex(domainName) && domain.ssl.sanDomains) {
        const covered = domain.ssl.sanDomains.some((san) => san.toLowerCase() === wwwHost.toLowerCase() ||
