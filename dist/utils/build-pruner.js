...
const removeBuildDir = async (buildPath) => {
    ...
    await withBackoffRetry(`Remove ${path.basename(buildPath)}`, () => fsExtra.remove(buildPath), 5, 2000);
};
/**
...
const removeBuilds = async (appName, buildPaths) => {
    ...
    for (const buildPath of buildPaths) {
        try {
            await Logger.spinner(`Removing old build: ${path.basename(buildPath)}`, () => removeBuildDir(buildPath));
            await AppRepo.removeBuild(appName, buildPath);
        }
        catch {
            Logger.warn(`Could not remove "${path.basename(buildPath)}" — skipping. Run ${Logger.command('dm clean')} to retry.`);
        }
    }
};
/**
 ...
 */
export const pruneOldBuilds = async (appName) => {
    const app = await AppRepo.findByName(appName);
    if (!app?.builds || app.builds.length <= MAX_BUILDS)
        return;
 ...
 */
export const pruneAllBuilds = async (appName) => {
    const app = await AppRepo.findByName(appName);
