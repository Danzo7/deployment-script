 ...
 */
export async function handleQuickConnect(rawArgs) {
    ...
    if (atHostIdx !== -1) {
        ...
        if (!hostPart) {
            Logger.error('Invalid @host syntax. Example: dm @10.10.10.10 deploy myapp');
            process.exit(1);
        }
        ...
        await connectRemote(host, port, identity, commandArgs.length > 0 ? commandArgs : undefined);
        return true;
    }
