 ...
 */
export const checkSvn = (throwOnMissing = false) => {
    let out = null;
    try {
...
export const handleSvnRepo = async ({ dir, repo, branch, }) => {
    checkSvn(true);
    const url = buildSvnUrl(repo, branch);
    const isWc = isSvnWorkingCopy(dir);
    if (!isWc && !isDirectoryEmpty(dir)) {
        throw new Error(`Please make sure the directory ${dir} is empty or is a valid SVN working copy.`);
    }
    if (!isWc) {
        Logger.info(`Checking out SVN repository ${url} into ${dir}`);
        await withRetry('Checking out SVN repository', async () => svn(`checkout "${url}" "${dir}"`, process.cwd()));
        Logger.success('Repository checked out successfully.');
        return true;
    }
    // Get revision before update
    const revBefore = await withRetry('Getting SVN revision', async () => svn('info --show-item revision', dir));
    Logger.info('Updating SVN working copy...');
    await withRetry('Updating SVN working copy', async () => svn('update', dir));
    const revAfter = await withRetry('Getting SVN revision after update', async () => svn('info --show-item revision', dir));
...
export const getLastSvnRevision = async (dir) => {
    try {
...
export const discardSvnChanges = async (dir) => {
    try {
        await withRetry('Reverting SVN changes', async () => svn('revert -R .', dir));
...
export const relocateSvnRepo = async (dir, newUrl, branch) => {
    checkSvn(true);
    const fullUrl = buildSvnUrl(newUrl, branch);
    try {
        Logger.info(`Relocating SVN repository to ${fullUrl}`);
        await withRetry('Relocating SVN repository', async () => svn(`relocate "${fullUrl}"`, dir));
