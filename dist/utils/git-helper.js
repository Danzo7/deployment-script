...
export const checkGit = () => {
    try {
        ...
        throw new Error('git is not installed or not on your PATH.\n' +
            '  → Install git from https://git-scm.com/downloads and re-run the command.');
    }
};
export const handleGitRepo = async ({ dir, repo, branch, }) => {
    checkGit();
    const git = simpleGit(dir);
    const isGitRepo = await withRetry('Checking Git status', () => git.checkIsRepo(CheckRepoActions.IS_REPO_ROOT));
    ...
    if (status.ahead > 0) {
        ...
        await withRetry('Resetting to remote', async () => git.reset(['--hard', `origin/${status.current}`]));
    }
    // Re-fetch status after potential reset so behind count is accurate
    const freshStatus = await withRetry('Getting updated repository status', async () => git.status());
...
export const pushChanges = async ({ dir, commitMessage, }) => {
    const git = simpleGit(dir);
    const status = await withRetry('Getting repository status', async () => git.status());
    if (status.behind > 0) {
    ...
    try {
        ...
        await withRetry('Committing changes', async () => git.commit(commitMessage));
...
export const getLastCommit = async (dir) => {
    const git = simpleGit(dir);
...
export const discardUncommittedChanges = async (dir) => {
    const git = simpleGit(dir);
    try {
        ...
        await withRetry('Restoring deleted tracked files', async () => git.checkout(['.']));
...
export const changeRemoteUrl = async (dir, newUrl, newBranch) => {
    checkGit();
    const git = simpleGit(dir);
    try {
        ...
        await withRetry('Changing git remote URL', async () => git.remote(['set-url', remoteName, newUrl]));
        ...
        if (newBranch) {
            ...
            await withRetry('Fetching new branch', async () => git.fetch([remoteName, newBranch]));
            // Check if branch exists locally
            const branches = await withRetry('Getting branches', async () => git.branchLocal());
            if (branches.all.includes(newBranch)) {
                // Branch exists locally, just checkout
                await withRetry('Checking out branch', async () => git.checkout(newBranch));
            }
            else {
                // Branch doesn't exist locally, create and track remote branch
                await withRetry('Checking out new branch', async () => git.checkout(['-b', newBranch, `${remoteName}/${newBranch}`]));
            }
            // Pull latest changes
 ...
 */
export const isGitRepo = async (url) => {
    try {
        const git = simpleGit();
