...
import fs from 'fs';
import { startRemoteServer, serverEvents, getActiveSessions, disconnectSession, } from '../utils/ssh-server.js';
import { RemoteServeDashboard } from './RemoteServeDashboard.js';
import { Logger } from '../utils/logger.js';
import { REMOTE_AUDIT_LOG_PATH } from '../constants.js';
const MAX_LOG = 200;
function App({ serverInfo, initialLogs, }) {
    const { exit } = useApp();
    ...
    useEffect(() => {
        if (!fs.existsSync(REMOTE_AUDIT_LOG_PATH))
            return;
    ...
    return (_jsx(RemoteServeDashboard, { bindAddress: serverInfo.bindAddress, port: serverInfo.port, fingerprint: serverInfo.fingerprint, sessions: sessions, logs: logs, onDisconnect: handleDisconnect, onQuit: handleQuit }));
}
export async function launchRemoteServe(port) {
    ...
    const { waitUntilExit } = render(_jsx(App, { serverInfo: serverInfo, initialLogs: pendingLogs }));
    await waitUntilExit();
