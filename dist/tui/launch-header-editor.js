...
export async function launchDomainHeaderEditor(domainName) {
    ...
    let savedCount = 0;
    const { waitUntilExit } = render(_jsx(HeaderEditor, { target: normalized, initial: initial, onSave: async (rows, count) => {
...
export async function launchRouteHeaderEditor(domainName, location) {
    ...
    Logger.isMuted = false;
    let savedCount = 0;
    const { waitUntilExit } = render(_jsx(HeaderEditor, { target: target, initial: initial, onSave: async (rows, count) => {
