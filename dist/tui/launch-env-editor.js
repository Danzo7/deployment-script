...
async function applyChanges(envDir, rows) {
...
export async function launchEnvEditor(appName) {
    pauseRepl();
    Logger.isMuted = true;
    ...
    let savedCount = 0;
    const { waitUntilExit } = render(_jsx(EnvEditor, { appName: appName, initial: initial, onSave: async (rows, count) => {
