...
const isGroup = (node) => node.kind === "group";
function coerce(raw, type) {
  if (type === "boolean") return raw === true || raw === "true";
  if (type === "number") return Number(raw);
  return String(raw);
}
function resolveLeafArgs(node, positionalTokens, flagTokens) {
  const args = {};
  const flagToKey = /* @__PURE__ */ new Map();
  const negatedKeys = /* @__PURE__ */ new Set();
  ...
  for (const rawFlag of Object.keys(flagTokens)) {
    ...
    if (key !== void 0 && rawFlag.startsWith("no-")) {
      negatedKeys.add(key);
    }
  }
  ...
  for (const [key, spec] of Object.entries(node.options ?? {})) {
    ...
    const raw = flagTokens[flagName] ?? (spec.alias ? flagTokens[spec.alias] : void 0);
    if (raw === void 0) {
    ...
    subcommands: {
      ...
      "set-cert": {
        ...
        handler: async ({ name, cert, key, pfx, password, force }) => {
          ...
          const hasPfx = Boolean(pfx || password !== void 0);
          if (hasPem && hasPfx)
            throw new Error(
...
async function ensureAppDirectories() {
  ...
  const { getRegisteredTypes: getRegisteredTypes2, getHandler } = await import("./app-types/index.js");
  const dirs = [APP_DIR, ...getRegisteredTypes2().map((t) => getHandler(t).getAppsDir())];
