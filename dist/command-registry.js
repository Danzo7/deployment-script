...
import { APP_DIR, SECRET_KEY, REMOTE_PORT, } from './constants.js';
...
import { remoteServe, remoteKeyAdd, remoteKeyRemove, remoteKeyList, remoteStatus, remoteConnect, remoteRenameUser, } from './commands/remote.js';
import { logClear } from './commands/log-clear.js';
import { metrics } from './commands/metrics.js';
export const isGroup = (node) => node.kind === 'group';
    ...
    init: {
        ...
        options: {
            ...
            vcs: {
                ...
                describe: 'Version control system to use (auto-detected if omitted; use "local" for a local folder path)',
            },
        },
        lockArg: 'name',
        handler: async ({ name, repo, branch, port, type, projectDir, vcs, }) => {
            await init({
                name,
                repo,
                branch,
            ...
            'set-cert': {
                ...
                options: {
                    ...
                    force: {
                        type: 'boolean',
                        default: false,
                        describe: 'Attach certificate even if it does not cover the domain name',
                    },
                },
                ...
                handler: async ({ name, cert, key, pfx, password, force }) => {
                    ...
                    if (hasPem && hasPfx)
                        throw new Error('Use either --cert/--key or --pfx/--password, not both.');
                    if (!hasPem && !hasPfx)
                ...
                positionals: [
                    {
                        name: 'username',
                        demandOption: true,
                        describe: 'The username whose key should be revoked (see "dm remote list")',
                    },
                ],
