...
async function addTraceabilityFields() {
  Logger.info(chalk.blue("\u{1F504} Adding traceability fields (createdBy/updatedBy)..."));
  const db = getDB();
  try {
    if (dbType === "sqlite") {
  ...
  for (const { table, columns } of migrations) {
    for (const { name, sql } of columns) {
      try {
        ...
        if (errorMsg.includes("duplicate column") || causeMsg.includes("duplicate column") || errorCode === "SQLITE_ERROR") {
