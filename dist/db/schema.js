...
const appsTableSqlite = sqliteTable(
  "apps",
  {
    ...
    updatedAt: integer("updatedAt", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`).$onUpdate(() => /* @__PURE__ */ new Date()),
    ...
    lastDeployedCommit: text("lastDeployedCommit")
    // JSON object
  },
  (table) => [
...
const storagesTableSqlite = sqliteTable(
  "storages",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    name: text("name").notNull().unique(),
...
const domainsTableSqlite = sqliteTable(
  "domains",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    name: text("name").notNull().unique(),
...
const routesTableSqlite = sqliteTable(
  "routes",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
...
const appStorageTableSqlite = sqliteTable(
  "app_storage",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    appId: integer("appId").notNull().references(() => appsTableSqlite.id, { onDelete: "cascade" }),
...
const appConfigTableSqlite = sqliteTable(
  "app_config",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    appId: integer("appId").notNull().unique().references(() => appsTableSqlite.id, { onDelete: "cascade" }),
...
const appsTablePostgres = pgTable(
  "apps",
  {
    ...
    updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow().$onUpdate(() => /* @__PURE__ */ new Date()),
    ...
    lastDeployedCommit: jsonb("last_deployed_commit")
    // JSON object
  },
  (table) => [
...
const storagesTablePostgres = pgTable(
  "storages",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    name: varchar("name", { length: 255 }).notNull().unique(),
...
const domainsTablePostgres = pgTable(
  "domains",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    name: varchar("name", { length: 255 }).notNull().unique(),
...
const routesTablePostgres = pgTable(
  "routes",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    domainId: uuid("domain_id").notNull().references(() => domainsTablePostgres.id, { onDelete: "cascade" }),
    path: varchar("path", { length: 500 }).notNull(),
    appId: uuid("app_id").notNull().references(() => appsTablePostgres.id, { onDelete: "cascade" }),
    createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow().$onUpdate(() => /* @__PURE__ */ new Date()),
    createdBy: varchar("created_by", { length: 255 }).notNull().default("system"),
    updatedBy: varchar("updated_by", { length: 255 }).notNull().default("system"),
...
const appStorageTablePostgres = pgTable(
  "app_storage",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    appId: uuid("app_id").notNull().references(() => appsTablePostgres.id, { onDelete: "cascade" }),
    storageId: uuid("storage_id").notNull().references(() => storagesTablePostgres.id, { onDelete: "cascade" }),
    createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow()
  },
  (table) => [
...
const appConfigTablePostgres = pgTable(
  "app_config",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    appId: uuid("app_id").notNull().unique().references(() => appsTablePostgres.id, { onDelete: "cascade" }),
...
const appsTable = dbType === "postgres" ? appsTablePostgres : appsTableSqlite;
const storagesTable = dbType === "postgres" ? storagesTablePostgres : storagesTableSqlite;
const domainsTable = dbType === "postgres" ? domainsTablePostgres : domainsTableSqlite;
const routesTable = dbType === "postgres" ? routesTablePostgres : routesTableSqlite;
const appStorageTable = dbType === "postgres" ? appStorageTablePostgres : appStorageTableSqlite;
const appConfigTable = dbType === "postgres" ? appConfigTablePostgres : appConfigTableSqlite;
const appsRelationsSqlite = relations(appsTableSqlite, ({ many, one }) => ({
...
const domainsRelationsSqlite = relations(
  domainsTableSqlite,
  ({ many }) => ({
    routes: many(routesTableSqlite)
  })
);
const routesRelationsSqlite = relations(
  routesTableSqlite,
  ({ one }) => ({
...
const storagesRelationsSqlite = relations(
  storagesTableSqlite,
  ({ many }) => ({
    appStorages: many(appStorageTableSqlite)
  })
);
const appStorageRelationsSqlite = relations(
...
const appConfigRelationsSqlite = relations(
...
const appsRelationsPostgres = relations(
  appsTablePostgres,
  ({ many, one }) => ({
...
const domainsRelationsPostgres = relations(
  domainsTablePostgres,
  ({ many }) => ({
    routes: many(routesTablePostgres)
  })
);
const routesRelationsPostgres = relations(
  routesTablePostgres,
  ({ one }) => ({
...
const storagesRelationsPostgres = relations(
  storagesTablePostgres,
  ({ many }) => ({
    appStorages: many(appStorageTablePostgres)
  })
);
const appStorageRelationsPostgres = relations(
...
const appConfigRelationsPostgres = relations(
...
export {
  appConfigRelationsPostgres,
  appConfigRelationsSqlite,
  appConfigTable,
  appConfigTablePostgres,
  appConfigTableSqlite,
  appStorageRelationsPostgres,
  appStorageRelationsSqlite,
  appStorageTable,
  appStorageTablePostgres,
  appStorageTableSqlite,
  appsRelationsPostgres,
  appsRelationsSqlite,
  appsTable,
  appsTablePostgres,
  appsTableSqlite,
  dbType,
  domainsRelationsPostgres,
  domainsRelationsSqlite,
  domainsTable,
  domainsTablePostgres,
  domainsTableSqlite,
  routesRelationsPostgres,
  routesRelationsSqlite,
  routesTable,
  routesTablePostgres,
  routesTableSqlite,
  storagesRelationsPostgres,
  storagesRelationsSqlite,
  storagesTable,
  storagesTablePostgres,
  storagesTableSqlite
};
