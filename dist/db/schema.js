import { sqliteTable, text, integer, index as sqliteIndex, } from 'drizzle-orm/sqlite-core';
import { pgTable, uuid, varchar, timestamp, integer as pgInteger, text as pgText, jsonb, index as pgIndex, } from 'drizzle-orm/pg-core';
...
export const appConfigTableSqlite = sqliteTable('app_config', {
    id: integer('id').primaryKey({ autoIncrement: true }),
    appId: integer('appId')
        .notNull()
        .unique()
...
// PostgreSQL Schema
// ============================================================================
export const appsTablePostgres = pgTable('apps', {
...
export const domainsTablePostgres = pgTable('domains', {
    id: uuid('id').primaryKey().defaultRandom(),
    name: varchar('name', { length: 255 }).notNull().unique(),
    createdAt: timestamp('created_at', { mode: 'date' }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { mode: 'date' })
...
export const routesTablePostgres = pgTable('routes', {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const appStorageTablePostgres = pgTable('app_storage', {
    id: uuid('id').primaryKey().defaultRandom(),
    appId: uuid('app_id')
        .notNull()
        .references(() => appsTablePostgres.id, { onDelete: 'cascade' }),
...
export const appConfigTablePostgres = pgTable('app_config', {
    id: uuid('id').primaryKey().defaultRandom(),
    appId: uuid('app_id')
        .notNull()
        .unique()
...
export const appsTable = dbType === 'postgres' ? appsTablePostgres : appsTableSqlite;
export const storagesTable = dbType === 'postgres' ? storagesTablePostgres : storagesTableSqlite;
export const domainsTable = dbType === 'postgres' ? domainsTablePostgres : domainsTableSqlite;
export const routesTable = dbType === 'postgres' ? routesTablePostgres : routesTableSqlite;
export const appStorageTable = dbType === 'postgres' ? appStorageTablePostgres : appStorageTableSqlite;
export const appConfigTable = dbType === 'postgres' ? appConfigTablePostgres : appConfigTableSqlite;
// ============================================================================
// Relations
// ============================================================================
// SQLite Relations
...
export const domainsRelationsSqlite = relations(domainsTableSqlite, ({ many }) => ({
    routes: many(routesTableSqlite),
}));
export const routesRelationsSqlite = relations(routesTableSqlite, ({ one }) => ({
...
export const storagesRelationsSqlite = relations(storagesTableSqlite, ({ many }) => ({
    appStorages: many(appStorageTableSqlite),
}));
export const appStorageRelationsSqlite = relations(appStorageTableSqlite, ({ one }) => ({
    app: one(appsTableSqlite, {
...
export const appConfigRelationsSqlite = relations(appConfigTableSqlite, ({ one }) => ({
    app: one(appsTableSqlite, {
...
export const appsRelationsPostgres = relations(appsTablePostgres, ({ many, one }) => ({
...
export const domainsRelationsPostgres = relations(domainsTablePostgres, ({ many }) => ({
    routes: many(routesTablePostgres),
}));
export const routesRelationsPostgres = relations(routesTablePostgres, ({ one }) => ({
...
export const storagesRelationsPostgres = relations(storagesTablePostgres, ({ many }) => ({
    appStorages: many(appStorageTablePostgres),
}));
export const appStorageRelationsPostgres = relations(appStorageTablePostgres, ({ one }) => ({
    app: one(appsTablePostgres, {
...
export const appConfigRelationsPostgres = relations(appConfigTablePostgres, ({ one }) => ({
    app: one(appsTablePostgres, {
