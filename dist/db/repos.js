...
import { appsTable, storagesTable, domainsTable, routesTable, appStorageTable, appConfigTable, dbType, } from './schema.js';
...
function toDate(timestamp) {
    ...
    if (typeof timestamp === 'number')
        return new Date(timestamp * 1000); // SQLite unix timestamp
...
function mapToApp(row) {
    return {
        ...
        lastDeployedCommit: deserializeJSON(row.lastDeployedCommit),
    };
}
...
export const AppRepo = {
    ...
    getAllWithStorages: async () => {
        ...
        return rows.map((row) => {
            ...
            const storages = (row.appStorages || []).map((as) => mapToStorage(as.storage));
            return { ...app, storages };
        });
    },
    /**
    ...
    findByNameWithConfig: async (name) => {
        ...
        const row = await db.query[dbType === 'postgres' ? 'appsTablePostgres' : 'appsTableSqlite'].findFirst({
            where: (apps, { eq }) => eq(apps.name, name),
            with: {
                config: true,
            },
        });
...
export const DomainRepo = {
    ...
    findByNameWithRoutes: async (name) => {
        ...
        const row = await db.query[dbType === 'postgres' ? 'domainsTablePostgres' : 'domainsTableSqlite'].findFirst({
            where: (domains, { eq }) => eq(domains.name, name),
            with: {
                routes: {
                    with: {
                        app: true,
...
export const RouteRepo = {
    ...
    getAllByDomainIdWithApp: async (domainId) => {
        const db = getDB();
        const rows = await db.query[dbType === 'postgres' ? 'routesTablePostgres' : 'routesTableSqlite'].findMany({
            where: (routes, { eq }) => eq(routes.domainId, domainId),
            with: {
                app: true,
            },
        });
    ...
    getAllByAppIdWithApp: async (appId) => {
        const db = getDB();
        const rows = await db.query[dbType === 'postgres' ? 'routesTablePostgres' : 'routesTableSqlite'].findMany({
            where: (routes, { eq }) => eq(routes.appId, appId),
            with: {
                app: true,
            },
        });
...
export const AppConfigRepo = {
    ...
    update: async (appId, data, updatedBy) => {
        ...
        if (data.instances !== undefined)
            updateFields.instances = data.instances;
        if (data.maxMemory !== undefined)
            updateFields.maxMemory = data.maxMemory;
        ...
        if ('maxRestarts' in data)
            updateFields.maxRestarts = data.maxRestarts ?? null;
        if ('minUptime' in data)
            updateFields.minUptime = data.minUptime ?? null;
        if ('restartDelay' in data)
            updateFields.restartDelay = data.restartDelay ?? null;
        if ('nodeArgs' in data)
            updateFields.nodeArgs = data.nodeArgs ?? null;
        if ('killTimeout' in data)
            updateFields.killTimeout = data.killTimeout ?? null;
        // Set updatedBy
        updateFields.updatedBy = updatedBy || 'system';
        await db
        ...
        if (!config)
            throw new Error('App config not found after update');
        return config;
    },
    delete: async (appId) => {
        const db = getDB();
