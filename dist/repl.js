...
import { setReplInterface, getActiveRl, setReplFactory, isHandingOff, } from './utils/repl-context.js';
import { COMMANDS, isGroup, resolveLeafArgs, ensureAppDirectories, } from './command-registry.js';
// ─── Tokeniser ────────────────────────────────────────────────────────────────
...
function parseTokens(tokens, optionSpecs) {
    ...
    if (optionSpecs) {
        for (const [key, spec] of Object.entries(optionSpecs)) {
            ...
            if (spec.alias)
                flagTypes.set(spec.alias, spec.type);
        }
    }
...
function buildLeafHelp(node) {
    ...
    if (node.options && Object.keys(node.options).length) {
        ...
        for (const [key, spec] of entries) {
            ...
            if (spec.choices)
                meta.push(`[choices: ${spec.choices.join(', ')}]`);
            if (spec.default !== undefined)
                meta.push(`[default: ${spec.default}]`);
            if (spec.demandOption)
                meta.push(chalk.red('[required]'));
...
function buildHelp() {
    ...
    for (const [key, node] of Object.entries(COMMANDS)) {
        ...
        if (isGroup(node)) {
            for (const [, subNode] of Object.entries(node.subcommands)) {
                if (isGroup(subNode))
                    continue; // no 2-level nesting today
    ...
    const sections = Array.from(groups.entries()).map(([group, lines]) => `  ${chalk.cyan(group)}\n${lines.join('\n')}`);
...
async function runStreamingInRepl(node, args) {
    await new Promise((resolveDone) => {
        ...
        const restore = async () => {
            ...
            process.removeAllListeners('SIGINT');
            for (const l of existingSigInt)
                process.on('SIGINT', l);
            Logger.nl();
            if (node.onStreamEnd)
                await node.onStreamEnd();
            resolveDone();
        };
...
// ─── REPL entry point ─────────────────────────────────────────────────────────
export async function startRepl(version) {
