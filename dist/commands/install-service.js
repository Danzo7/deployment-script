...
async function installWindows() {
    ...
    await runCommand('schtasks', [
        ...
        '/SC',
        'ONSTART', // trigger: at system startup
        '/DELAY',
        '0000:10', // 10-second delay after boot for network
        '/RU',
        'SYSTEM', // run as SYSTEM (no user session needed)
        '/TN',
        SCHTASKS_TASK_NAME,
        '/TR',
        taskRun,
        '/RL',
        'HIGHEST', // highest privilege level
...
async function uninstallWindows() {
    ...
    assertWindowsPrivileges();
    await runCommand('schtasks', ['/Delete', '/F', '/TN', SCHTASKS_TASK_NAME], `Removing task "${SCHTASKS_TASK_NAME}"`, { ignoreErrors: true });
