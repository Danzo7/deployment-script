...
`);
  ...
  if (params.instances !== void 0) {
    ...
    if (params.instances < 1) throw new Error("Instances must be at least 1");
    updates.instances = params.instances;
    needsRestart = true;
  }
  if (params.memory !== void 0) {
    validateMemoryFormat(params.memory);
    updates.maxMemory = params.memory;
    needsRestart = true;
  }
  if (params.nodeArgs !== void 0) {
  ...
  if (params.minUptime !== void 0) {
    if (params.minUptime) validateTimeFormat(params.minUptime);
    updates.minUptime = params.minUptime || null;
    needsRestart = true;
  }
  if (params.autorestart !== void 0) {
    updates.autorestart = params.autorestart;
    needsRestart = true;
  }
  if (params.maxRestarts !== void 0) {
    if (params.maxRestarts < 0) throw new Error("Max restarts must be 0 or greater");
    updates.maxRestarts = params.maxRestarts;
    needsRestart = true;
  }
  if (params.restartDelay !== void 0) {
    if (params.restartDelay < 0) throw new Error("Restart delay must be 0 or greater (milliseconds)");
    updates.restartDelay = params.restartDelay;
    needsRestart = true;
  }
  if (params.killTimeout !== void 0) {
    if (params.killTimeout < 0) throw new Error("Kill timeout must be 0 or greater (milliseconds)");
    if (params.killTimeout > 0 && params.killTimeout < 1e3) {
  ...
  if (updates.instances !== void 0) Logger.info(`  Instances: ${app.config.instances} \u2192 ${Logger.highlight(updatedConfig.instances.toString())}`);
  if (updates.maxMemory !== void 0) Logger.info(`  Max Memory: ${app.config.maxMemory} \u2192 ${Logger.highlight(updatedConfig.maxMemory)}`);
  if (updates.autorestart !== void 0) Logger.info(`  Auto Restart: ${app.config.autorestart ?? "default"} \u2192 ${Logger.highlight(String(updatedConfig.autorestart))}`);
  if (updates.maxRestarts !== void 0) Logger.info(`  Max Restarts: ${app.config.maxRestarts ?? "default"} \u2192 ${Logger.highlight(updatedConfig.maxRestarts?.toString() ?? "default")}`);
  if (updates.minUptime !== void 0) Logger.info(`  Min Uptime: ${app.config.minUptime ?? "default"} \u2192 ${Logger.highlight(updatedConfig.minUptime ?? "default")}`);
  if (updates.restartDelay !== void 0) Logger.info(`  Restart Delay: ${app.config.restartDelay ?? "default"} \u2192 ${Logger.highlight(updatedConfig.restartDelay?.toString() ?? "default")}ms`);
  if (updates.killTimeout !== void 0) Logger.info(`  Kill Timeout: ${app.config.killTimeout ?? "default"} \u2192 ${Logger.highlight(updatedConfig.killTimeout?.toString() ?? "default")}ms`);
  if (updates.nodeArgs !== void 0) Logger.info(`  Node Args: ${app.config.nodeArgs ?? "none"} \u2192 ${Logger.highlight(updatedConfig.nodeArgs ?? "none")}`);
  if (needsRestart) {
    ...
    if (status === "online") {
      ...
      if (!activeBuild) throw new Error(`No active build found for ${name}. Deploy the app first.`);
