...
export async function routeRemoveHeader(domainName, location, key) {
    ...
    if (!route.headers || !(key in route.headers)) {
        throw new Error(`Header "${key}" is not set on route "/${normalizedPath}" of domain "${normalizedDomain}"`);
    }
    ...
    // 7. Log success
    Logger.success(`Header "${key}" removed from route "/${normalizedPath}" of domain "${normalizedDomain}".`);
}
