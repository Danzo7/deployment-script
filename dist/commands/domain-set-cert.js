...
import { checkOpenssl, extractPfx, validateCertAndKey, certCoversHostname, parseCertMetadata, writeCertFiles, buildSSLConfig, } from '../utils/ssl-helper.js';
import { DomainRepo } from '../db/repos.js';
import { Logger } from '../utils/logger.js';
import { getCurrentUser } from '../utils/user-context.js';
export async function domainSetCert(name, opts) {
    ...
    if (domain.ssl.mode === 'letsencrypt') {
        throw new Error("Let's Encrypt mode is not yet supported. Set ssl mode to 'custom' first.");
    }
...
async function _pemPath(normalized, domain, opts) {
    ...
    if (!covers) {
        ...
        if (!opts.force) {
            throw new Error(`Certificate does not cover "${normalized}". Certificate covers: ${sanList}.\nUse --force to attach anyway.`);
        }
        Logger.warn(`Certificate does not cover "${normalized}". Certificate covers: ${sanList}. Attaching anyway (--force).`);
    }
    // 9. Warn if domain already has a cert attached
    if (domain.ssl.certPath) {
        Logger.warn(`Replacing existing certificate for "${normalized}" (previously expires: ${domain.ssl.expiresAt ?? 'unknown'}).`);
    }
    ...
    // 13. Log success
    Logger.success(`Certificate attached to "${normalized}". Expires: ${metadata.expiresAt}.`);
}
