...
export const init = async ({ name, repo, branch, port, type, projectDir, vcsType, }) => {
    if (!repo)
        throw new Error('Repository URL or local folder path is required.');
    const resolvedVcsType = vcsType ?? await detectVcsType(repo);
    if (!vcsType) {
        Logger.info(`VCS type not specified — detected: ${Logger.highlight(resolvedVcsType)}`);
    }
    if (resolvedVcsType === 'svn')
        checkSvn();
    if (resolvedVcsType === 'git')
        checkGit();
    if (resolvedVcsType === 'local')
        checkLocalFolder(repo);
    ...
    const resolvedType = type ?? await detectProjectType({
        appName: name,
        repo,
        branch,
        projectDir,
    ...
    Logger.advice(`Next steps: Run ${Logger.command(`dm deploy ${name}`)} to deploy the app. Use ${Logger.command(`dm list`)} to verify its status.`);
};
