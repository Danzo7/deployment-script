...
export const info = async ({ name }) => {
    ...
    Logger.row('Last Deploy', chalk.yellow(formatDate(app.lastDeploy, chalk.gray('Never'))));
    Logger.divider();
    ...
    if (app.storages.length > 0) {
        ...
        for (const storage of app.storages) {
            ...
            Logger.row('Storage', `${chalk.white(storage.name)} ${chalk.gray('(')}${chalk.green(size)}${chalk.gray(')')}`);
        }
    }
    // Routes & Domains
