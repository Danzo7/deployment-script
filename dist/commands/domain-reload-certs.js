...
import { getCertStorePath, loadCertFromStore, buildSSLConfig, } from '../utils/ssl-helper.js';
/**
 ...
 */
export async function domainReloadCerts(name) {
    if (name) {
        ...
        Logger.print(chalk.gray(`  Added: ${added}, Updated: ${updated}, Removed: ${removed}, Skipped: ${skipped}`));
        Logger.nl();
    }
}
/**
...
async function reloadDomainCert(domainName, quiet = false) {
    ...
    if (dbHasCert && (!certExists || !keyExists)) {
        const missing = [];
        if (!certExists)
            missing.push('cert.pem');
        if (!keyExists)
            missing.push('key.pem');
        Logger.warn(`Certificate files missing for "${domainName}": ${missing.join(', ')}. Removing SSL configuration from database.`);
