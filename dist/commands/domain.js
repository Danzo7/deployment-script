...
export async function domainAdd(name) {
...
export async function domainRemove(name, force) {
    const normalized = normalizeDomainName(name);
    ...
    if (routes.length > 0 && !force) {
        const routeLines = routes
            ...
            .join('\n');
        throw new Error(`Domain "${normalized}" has routes:\n${routeLines}\nUse --force to cascade delete.`);
    }
    if (routes.length > 0 && force) {
...
function sslColumnValue(domain) {
...
function expiryColored(expiresAt) {
    ...
    const now = new Date();
    const warningThreshold = new Date(now.getTime() + CERT_EXPIRY_WARNING_DAYS * 24 * 60 * 60 * 1000);
...
export async function getAppRouteLines(appName) {
    ...
    for (const route of routes) {
        ...
        if (ssl.mode === 'none') {
            ...
            if (expiry < now)
                sslLabel = chalk.red(`SSL expired ${distance}`);
            else if (expiry < warning)
                sslLabel = chalk.yellow(`SSL expires ${distance}`);
            else
                sslLabel = chalk.green(`SSL valid · expires ${distance}`);
        }
...
export async function domainList() {
    ...
    domains.forEach((domain, index) => {
        ...
        if (domain.lastPushedAt) {
            ...
            const isStale = domain.lastCompiledAt &&
...
export async function domainShow(name) {
    const normalized = normalizeDomainName(name);
    ...
    Logger.divider();
    if (domain.lastPushedAt) {
        Logger.row('Last pushed', chalk.yellow(`${formatDate(domain.lastPushedAt)} (${formatRelative(domain.lastPushedAt)})`));
    }
    else {
    ...
    if (domain.lastCompiledAt && !domain.lastPushedAt) {
        Logger.nl();
        Logger.print(`  ${chalk.yellow('⚠')} Config has been compiled but not yet pushed. Run ${chalk.cyan(`'dm domain push ${name}'`)} to deploy.`);
    }
    else if (domain.lastCompiledAt &&
        domain.lastPushedAt &&
        new Date(domain.lastCompiledAt) > new Date(domain.lastPushedAt)) {
        Logger.nl();
        Logger.print(`  ${chalk.yellow('⚠')} Config is stale — recompiled since last push. Run ${chalk.cyan(`'dm domain push ${name}'`)} to update.`);
    }
    Logger.divider();
    if (domain.routes.length === 0) {
        ...
        for (const route of domain.routes) {
            Logger.print(`  ${chalk.white('/' + route.path)}  →  ${chalk.whiteBright(route.app.name)}`);
        }
    }
    Logger.nl();
}
