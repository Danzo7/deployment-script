...
async function domainRemove(name, force) {
  const normalized = normalizeDomainName(name);
...
function sslColumnValue(domain) {
  if (domain.ssl.mode === "custom") {
    return domain.ssl.certPath ? chalk.green("custom \u2713") : chalk.yellow("custom (no cert)");
  }
  return domain.ssl.mode === "none" ? chalk.gray("none") : chalk.white(domain.ssl.mode);
}
...
async function getAppRouteLines(appName) {
...
async function domainList() {
  ...
  domains.forEach((domain, index) => {
    ...
    if (domain.lastPushedAt) {
      ...
      pushedValue = isStale ? chalk.yellow(`\u26A0 ${relativeTime}`) : chalk.green(relativeTime);
    } else {
...
async function domainShow(name) {
  const normalized = normalizeDomainName(name);
