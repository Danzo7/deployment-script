...
function handleLine(line, label) {
    ...
    if (trimmed.startsWith('{')) {
        ...
        const m = trimmed.match(/^(\S+)\s+-\s+\S+\s+\[([^\]]+)\]\s+"(\w+)\s+(\S+)\s+[^"]*"\s+(\d+)\s+(\d+)/);
        if (m) {
...
async function tailRemote(logPath, label, onError) {
    ...
    const stop = conn.execStream(cmd, (line) => handleLine(line, label), onError);
    return stop;
}
// ─── Command ──────────────────────────────────────────────────────────────────
export const metrics = async ({ name }) => {
    ...
    const unpushedDomains = [...new Set(routes.filter((r) => !r.domain.lastPushedAt).map((r) => r.domain.name))];
    ...
    Logger.info(`Streaming nginx logs for "${Logger.highlight(name)}"${isRemote ? ` via ${NGINX_REMOTE_HOST}` : ''} (Ctrl+C to stop)...\n`);
    const stopFns = [];
