...
import { normalizeDomainName, normalizePath, assertPathUnique, assertAppUniqueOnDomain, assertAppNotRoutedElsewhere, } from '../utils/route-validation.js';
import { getCurrentUser } from '../utils/user-context.js';
export async function routeAdd(appName, domainName, location, force) {
    ...
    Logger.success(`App ${Logger.highlight(appName)} routed to ${Logger.highlight(normalizedDomain)} at ${Logger.highlight('/' + normalizedPath)}.`);
}
export async function routeRemove(domainName, location) {
    const normalizedDomain = normalizeDomainName(domainName);
    const normalizedPath = normalizePath(location);
    const domain = await DomainRepo.findByName(normalizedDomain);
    ...
    Logger.success(`Route ${Logger.highlight('/' + normalizedPath)} on ${Logger.highlight(normalizedDomain)} removed.`);
}
export async function routeList(domainName) {
