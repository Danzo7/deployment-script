...
export const storageNew = async (name, linkName) => {
    ...
    Logger.success(`Storage ${Logger.highlight(name)} created at ${Logger.highlight(storagePath)} (symlink name: ${Logger.highlight(effectiveLinkName)}).`);
};
export const storageAttach = async (appName, storageName) => {
            ...
            if (stat !== null) {
                if (stat.isSymbolicLink()) {
                    ...
                    if (currentTarget !== storage.path) {
                        throw new Error(`Cannot attach: a symlink "${effectiveLinkName}" already exists in the active build pointing to a different target ("${currentTarget}").`);
                    }
                    // Correct symlink — no conflict, fall through
                }
                else {
                    throw new Error(`Cannot attach: a real directory "${effectiveLinkName}" already exists in the active build. Remove or rename it first.`);
                }
            }
        }
    }
    ...
    Logger.success(`Storage ${Logger.highlight(storageName)} attached to ${Logger.highlight(appName)} as ${Logger.highlight(storage.linkName ?? storage.name)}.`);
};
export const storageDetach = async (appName, storageName) => {
    const app = await AppRepo.findByName(appName);
    const storage = await StorageRepo.findByName(storageName);
    ...
    Logger.success(`Storage ${Logger.highlight(storageName)} detached from ${Logger.highlight(appName)}.`);
};
export const storageRm = async (name) => {
    ...
    if (attachedApps.length > 0) {
        ...
        throw new Error(`Storage "${name}" is still attached to the following apps: ${appNames.join(', ')}. Detach it from all apps before removing.`);
    }
    ...
    if (fs.existsSync(storagePath)) {
        ...
        Logger.info(`Storage directory ${Logger.highlight(storagePath)} not found on disk; skipping removal.`);
    }
...
export const storageLs = async () => {
    ...
    storages.forEach((storage, index) => {
        ...
        const attachedAppsDisplay = attachedApps.length > 0
