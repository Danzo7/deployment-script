...
const changeRepo = async ({
  ...
  Logger.advice(
    `The repository ${newRepo ? "URL " : ""}${newRepo && newBranch ? "and " : ""}${newBranch ? "branch " : ""}${newRepo || newBranch ? "has" : "have"} been updated. Run ${Logger.command(`dm deploy ${name}`)} to deploy from the ${newBranch ? "new branch" : "new repository"}.`
  );
};
export {
  changeRepo
