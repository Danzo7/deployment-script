...
export const listApps = async (filterType, showStorages, showRoutes) => {
    ...
    if (showStorages && showRoutes) {
        ...
        const longestName = Math.max(...apps.map((a) => a.name.length), 'Name'.length);
        const nameW = Math.min(Math.max(longestName + 2, 20), // at least 20
        35 // never exceed 35
        );
    ...
    for (const app of apps) {
        const statusColor = app.status === 'online'
            ? chalk.green
            : app.status === 'offline'
                ? chalk.red
                : app.status === 'stopped'
        ...
        if (showStorages) {
            const storageDisplay = app.storages && app.storages.length > 0
                ? app.storages.map((s) => s.name).join('\n')
                : chalk.gray('None');
            row.push(storageDisplay);
        }
        if (showRoutes) {
            const routesDisplay = app.routeDisplay && app.routeDisplay.length > 0
